...
public class StudentController {
    ...
    public ApiResult findAll(@PathVariable Integer page, @PathVariable Integer size) {
        ...
        IPage<User> res = studentService.findAll(studentPage);
        return  ApiResultHandler.buildApiResult(200,"分页查询所有学生",res);
    }

    @GetMapping("/student/{studentId}")
    public ApiResult findById(@PathVariable("studentId") Integer studentId) {
        User res = studentService.findById(studentId);
        if (res != null) {
    ...
    public ApiResult updatePwd(@RequestBody User student) {
        studentService.updatePwd(student);
        return ApiResultHandler.buildApiResult(200,"密码更新成功",null);
    }
    @PutMapping("/student")
    public ApiResult update(@RequestBody User student) {
    ...
    @PostMapping("/student")
    public ApiResult add(@RequestBody User student) {
